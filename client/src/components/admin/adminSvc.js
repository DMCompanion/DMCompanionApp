angular.module('companion')
  .service('adminSvc', function($http) {
    ...
    this.filterFeatures = (feature) => {
      let approvedFeatures = [];
      let unapprovedFeatures = [];
      let unapprovedCounter = 0;
      for (var i = 0; i < feature.length; i++) {
        if (feature[i].approved) {
          approvedFeatures.push(feature[i]);
        } else if (i === feature.length && unapprovedCounter === 0) {
          $scope.hasUnapprovedFeatures = false;
        } else {
          unapprovedFeatures.push(feature[i]);
          unapprovedCounter++;
          $scope.hasUnapprovedFeatures = true;
        }
      }
